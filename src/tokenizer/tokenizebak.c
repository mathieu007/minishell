...
// static int32_t	add_other_token2(char *token_str, t_token_type type,
// 	int32_t i, t_token_sequence *group)
// {
// 	t_token			*token;
...
// 	add_other_token(ft_strdup(""), TK_START, 0, group);
// 	while (str[i])
// 	{
// 		type = get_token_type(&str[i]);
// 		t_len = get_token_len(&str[i], type, false);
