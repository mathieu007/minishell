...
/*   Updated: 2023/06/22 15:48:21 by mroy             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int32_t	add_token_single_quote(char *str, int32_t i, t_token *parent)
{
	...
	token = add_tk("'", TK_SINGLEQUOTE, i, parent);
...
int32_t	add_token_double_quote(char *str, int32_t i, t_token *parent)
{
	...
	token = add_tk("\"", TK_DOUBLEQUOTE, i, parent);
		...
		bool inside_dbl_quotes)
{
	...
	token = add_tk("$(", TK_COMMANDSUBSTITUTION_OPEN, i, parent);
	i = goto_closing_parenthese(str, i);
	if (str[i] != ')')
		token->is_continuation = true;
	else
		add_tk(")", TK_COMMANDSUBSTITUTION_CLOSE, i, parent);
...
{
	add_tk("(", TK_PARENTHESE_OPEN, i, parent);
	i = goto_closing_parenthese(str, i);
	if (str[i] != ')')
