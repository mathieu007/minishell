...
char	*get_output2(char *full_path, char *filename, char *arg1, char *arg2,
...
void	run_test(char *command)
{
	...
	bash_err_output = get_output2("bash", "bash", "-c", command, STDERR_FILENO,
			STDOUT_FILENO);
	// snprintf(bash_cmd, MAX_COMMAND_LENGTH, "%s", command2);
	bash_output = get_output2("bash", "bash", "-c", command, STDOUT_FILENO,
			STDERR_FILENO);
	bash_status = return_code;
	// snprintf(minishell_command, MAX_COMMAND_LENGTH, "%s", command2);
	minishell_err_output = get_output2(full_name, "./minishell", command, NULL,
			STDERR_FILENO, STDOUT_FILENO);
	// snprintf(minishell_command, MAX_COMMAND_LENGTH, "%s", command2);
	minishell_output = get_output2(full_name, "./minishell", command, NULL,
