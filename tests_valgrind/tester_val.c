...
//	replacing occurrences of the search string with the replace string
...
// 		snprintf(minishell_command, MAX_COMMAND_LENGTH,
//				"valgrind --track-fds=yes --leak-check=full --track-origins=yes --show-reachable=yes ./minishell '%s'",
//				*command2);
// 		free(*command2);
// 		command2++;
// 		// while (*command2)
// 		// {
// 		// 	*command2 = replaceString(*command2, "'", "'\\''");
// 		// 	snprintf(minishell_command, MAX_COMMAND_LENGTH, "%s '%s'",
//					&minishell_command[0], *command2);
// 		// 	free(*command2);
// 		// 	command2++;
// 		// }
// 		// snprintf(minishell_command, MAX_COMMAND_LENGTH, "%s 2>&1 | grep
//				-e 'lost: ' -e 'still reachable: '", &minishell_command[0]);
...
// 			minishell_output = realloc(minishell_output, minishell_output_len
//					+ line_len + 1);
